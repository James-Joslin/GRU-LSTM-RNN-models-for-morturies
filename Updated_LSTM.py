...
if yes_or_no("Use pre-existing model?") == True:
    ...
    model = load_model(model_name="Deaths_6months_LSTM_V1")       
    print(model.summary())                                                                                                     
    temp = Daily
    ...
    plot_verification(
        ...
        daily2_reshape = daily2_reshape, TIME_STEPS=TIME_STEPS,
        y_train=y_train, y_test = y_test, timeseries=temp)
    save_model(model_In=model, save_name="Deaths_6months_LSTM_V1")
...
forecast_dataframe=pd.DataFrame({'LSTM':forecast})
forecast_dataframe.to_csv("./forecasted/lstm_forecast.csv", index = False)
plt.plot(forecast, 'g', label = "LSTM")
plt.legend()
plt.show()